package UrlSniper::App::Utils::ETL::JSONHandler ; 
...
sub doReadJSONFile {
   ...
   $dat = decode_json($json);
   
   # Output to screen one of the values read
   # print "Boss' hobbies: " .
   # $dat->{'boss'}->{'Hobbies'}->[0] . "n";
   # Modify the value, and write the output file as json
   # $dat->{'boss'}->{'Hobbies'}->[0] = "Swimming";
   # open my $fh, ">", "dat_out.json";
   # print $fh encode_json($dat);
   # close $fh;
   $ret = 0  ; 
